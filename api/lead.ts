...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/foundation\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Foundation Repair`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Foundation Repair',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Foundation Alerts <alerts@mursenmaintenance.com>',
      to: ['hello@scalesolving.com'],
      subject: '🏗️ New Lead: ' + fullName + (page_path && page_path !== '/' ? ' — ' + page_path : ''),
      html,
    });
